#ifndef ORIGINAL_BENCHMARK_TASK_DELEGATOR_LEGACY_H
#define ORIGINAL_BENCHMARK_TASK_DELEGATOR_LEGACY_H
...
namespace original
{
    // Legacy global-queue thread pool kept only for benchmark comparison.
    class taskDelegatorLegacy
    {
    public:
        // ==================== Task Base Interface ====================
    ...
    public:
        taskDelegatorLegacy(const taskDelegatorLegacy&) = delete; ///< Disable copy constructor
        taskDelegatorLegacy& operator=(const taskDelegatorLegacy&) = delete; ///< Disable copy assignment
        taskDelegatorLegacy(taskDelegatorLegacy&&) = delete; ///< Disable move constructor
        taskDelegatorLegacy& operator=(taskDelegatorLegacy&&) = delete; ///< Disable move assignment

        /**
         * @brief Constructs a task delegator with a given number of threads
         * @param thread_cnt Number of threads (default: 8)
         */
        explicit taskDelegatorLegacy(u_integer thread_cnt = 8);
...
template <typename TYPE>
template <typename Callback, typename... Args>
original::taskDelegatorLegacy::task<TYPE>::task(Callback&& c, Args&&... args)
...
template <typename TYPE>
void original::taskDelegatorLegacy::task<TYPE>::run()
{
    this->p.run();
}

template <typename TYPE>
original::async::future<TYPE> original::taskDelegatorLegacy::task<TYPE>::getFuture()
{
...
template <typename COUPLE>
bool original::taskDelegatorLegacy::taskComparator<COUPLE>::operator()(const COUPLE& lhs, const COUPLE& rhs) const
{
    return static_cast<u_integer>(lhs.second()) < static_cast<u_integer>(rhs.second());
}

inline void original::taskDelegatorLegacy::workerThread()
{
    while (true)
    {
...
inline original::u_integer original::taskDelegatorLegacy::moveAllDeferred()
{
...
inline original::taskDelegatorLegacy::taskDelegatorLegacy(const u_integer thread_cnt)
    : threads_(thread_cnt),
...
template <typename Callback, typename... Args>
auto original::taskDelegatorLegacy::submit(Callback&& c, Args&&... args)
{
    return this->submit(priority::NORMAL, std::forward<Callback>(c), std::forward<Args>(args)...);
}

template <typename Callback, typename... Args>
auto original::taskDelegatorLegacy::submit(const priority priority, Callback&& c, Args&&... args)
{
...
template <typename Callback, typename... Args>
auto original::taskDelegatorLegacy::submit(time::duration timeout, Callback&& c, Args&&... args)
{
    using ReturnType = decltype(c(args...));
    strongPtr<task<ReturnType>> new_task = makeStrongPtr<task<ReturnType>>(
        std::forward<Callback>(c),
    ...
    auto f = new_task->getFuture();
    {
        ...
        if (this->stopped_)
        {
            throw sysError("taskDelegatorLegacy already stopped");
        }
...
inline original::u_integer original::taskDelegatorLegacy::waitingCnt() const noexcept
{
    return this->tasks_waiting_.size();
}

inline original::u_integer original::taskDelegatorLegacy::immediateCnt() const noexcept
{
...
original::taskDelegatorLegacy::submit(const priority priority, strongPtr<task<TYPE>> t)
{
    auto f = t->getFuture();
    {
        uniqueLock lock{this->mutex_wait_};
...
inline void original::taskDelegatorLegacy::runDeferred()
{
...
inline void original::taskDelegatorLegacy::runAllDeferred()
{
    this->condition_.notifySome(this->moveAllDeferred());
}

inline bool original::taskDelegatorLegacy::discardDeferred()
{
    return this->tasks_deferred_.tryPop().hasValue();
}

inline void original::taskDelegatorLegacy::discardAllDeferred()
{
    this->tasks_deferred_.clear();
}

inline original::u_integer original::taskDelegatorLegacy::deferredCnt() const noexcept
{
    return this->tasks_deferred_.size();
}

inline void original::taskDelegatorLegacy::stop(const stopMode mode)
{
    if (this->stopped_)
        return;
...
inline bool original::taskDelegatorLegacy::isStopped() const noexcept
{
    return *this->stopped_;
}

inline original::u_integer original::taskDelegatorLegacy::activeThreads() const noexcept
{
    return *this->active_threads_;
}

inline original::u_integer original::taskDelegatorLegacy::idleThreads() const noexcept
{
    return *this->idle_threads_;
}

inline original::taskDelegatorLegacy::~taskDelegatorLegacy()
{
...
#endif // ORIGINAL_BENCHMARK_TASK_DELEGATOR_LEGACY_H
