 ...
 * @note Non-recursive lock - attempting to relock from same thread will
 * deadlock
 *
 * Example usage:
 * @code
 ...
 */
template <typename... MUTEX> class multiLock final : public lockGuard {
   ...
   */
  template <u_integer... IDXES> void lockAll(indexSequence<IDXES...> sequence);
...
inline original::pMutex::pMutex() : mutex_{} {
  if (const int code = pthread_mutex_init(&this->mutex_, nullptr); code != 0) {
    throw sysError("Failed to initialize mutex (pthread_mutex_init returned " +
...
inline void *original::pMutex::nativeHandle() noexcept { return &this->mutex_; }

inline void original::pMutex::lock() {
  if (const int code = pthread_mutex_lock(&this->mutex_); code != 0) {
    throw sysError("Failed to lock mutex (pthread_mutex_lock returned " +
                   printable::formatString(code) + ")");
  }
}

inline bool original::pMutex::tryLock() {
  if (const int code = pthread_mutex_trylock(&this->mutex_); code != 0) {
    if (code == EBUSY)
      return false;
...
inline void original::pMutex::unlock() {
  if (const int code = pthread_mutex_unlock(&this->mutex_); code != 0) {
    throw sysError("Failed to unlock mutex (pthread_mutex_unlock returned " +
                   printable::formatString(code) + ")");
  }
}

inline original::pMutex::~pMutex() {
  if (const int code = pthread_mutex_destroy(&this->mutex_); code != 0) {
    std::cerr << "Fatal error: Failed to destroy mutex (pthread_mutex_destroy "
                 "returned "
              << code << ")" << std::endl;
    std::terminate();
  }
...
#elif ORIGINAL_COMPILER_MSVC
inline original::wMutex::wMutex() { InitializeSRWLock(&this->lock_); }

inline original::ul_integer original::wMutex::id() const {
  return reinterpret_cast<ul_integer>(&this->lock_);
}

inline void *original::wMutex::nativeHandle() noexcept { return &this->lock_; }

inline void original::wMutex::lock() { AcquireSRWLockExclusive(&this->lock_); }
...
inline void original::mutex::lock() { this->mutex_.lock(); }

inline bool original::mutex::tryLock() { return this->mutex_.tryLock(); }

inline void original::mutex::unlock() { this->mutex_.unlock(); }

inline original::uniqueLock::uniqueLock(mutexBase &mutex,
...
inline original::uniqueLock::~uniqueLock() { this->unlock(); }
...
bool original::multiLock<MUTEX...>::tryLockAll(indexSequence<IDXES...>) {
  ...
  if (!success) {
    idx = 0;
    ((lock_status[idx] ? this->m_list.template get<IDXES>()->unlock() : void(),
      ++idx),
     ...);
  }
...
template <typename... MUTEX>
original::multiLock<MUTEX...>::multiLock(const lockPolicy policy,
                                         MUTEX &...mutex)
...
template <typename... MUTEX> void original::multiLock<MUTEX...>::lock() {
...
template <typename... MUTEX> bool original::multiLock<MUTEX...>::tryLock() {
  if (this->is_locked_all)
...
template <typename... MUTEX> void original::multiLock<MUTEX...>::unlock() {
  if (this->is_locked_all)
    this->unlockAll(makeReverseSequence<sizeof...(MUTEX)>());
}

template <typename... MUTEX> original::multiLock<MUTEX...>::~multiLock() {
  this->unlock();
}
