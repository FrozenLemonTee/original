...
/**
 * @file condition.h
 * @brief Cross-platform condition variable implementation for thread
 * synchronization
 ...
 *
 * Platform Support:
 * - GCC/Clang: Uses pthread_cond_t for condition variable implementation
 * (pCondition)
 ...
 */
class conditionBase : public noMeta {
public:
  /// Default constructor
   ...
   * @throws sysError if wait operation fails
   * @note Implements the "wait with predicate" pattern to avoid spurious
   * wakeups
   */
   ...
   */
  template <typename Pred>
  bool waitFor(mutexBase &mutex, const time::duration &d,
               Pred predicate) noexcept(noexcept(predicate()));
   ...
   *       control over thread notification, use notify() or notifyAll()
   * directly. The default threshold of 24 is chosen as a balance between the
...
template <typename Pred>
void original::conditionBase::wait(mutexBase &mutex, Pred predicate) noexcept(
    noexcept(predicate())) {
  while (!predicate()) {
    this->wait(mutex);
  }
...
template <typename Pred>
bool original::conditionBase::waitFor(
    mutexBase &mutex, const time::duration &d,
    Pred predicate) noexcept(noexcept(predicate())) {
...
inline original::pCondition::pCondition() : cond_{} {
  if (const int code = pthread_cond_init(&this->cond_, nullptr); code != 0) {
    throw sysError(
        "Failed to initialize condition variable (pthread_cond_init returned " +
        printable::formatString(code) + ")");
  }
}

inline void original::pCondition::wait(mutexBase &mutex) {
  staticError<valueError, !std::is_same_v<mutex::native_handle,
                                          pMutex::native_handle>>::asserts();
...
inline bool original::pCondition::waitFor(mutexBase &mutex,
  ...
  if (code == ETIMEDOUT)
    return false;
  throw sysError("Failed to timed wait on condition variable "
                 "(pthread_cond_timed-wait returned " +
                 printable::formatString(code) + ")");
}
...
inline void original::pCondition::notifyAll() {
  if (const int code = pthread_cond_broadcast(&this->cond_); code != 0) {
    throw sysError("Failed to broadcast condition variable "
                   "(pthread_cond_broadcast returned " +
                   printable::formatString(code) + ")");
  }
}
...
inline original::pCondition::~pCondition() {
  if (const int code = pthread_cond_destroy(&this->cond_); code != 0) {
    std::cerr << "Warning: Failed to destroy condition variable "
                 "(pthread_cond_destroy returned "
              << code << ")" << std::endl;
  }
}
#elif ORIGINAL_COMPILER_MSVC
...
inline void original::wCondition::wait(mutexBase &mutex) {
  if (const auto handle =
          static_cast<mutex::native_handle *>(mutex.nativeHandle());
      !SleepConditionVariableSRW(&this->cond_, handle, INFINITE, 0)) {
    throw sysError("Failed to wait on condition variable "
                   "(SleepConditionVariableSRW returned error " +
                   printable::formatString(GetLastError()));
  }
}

inline bool original::wCondition::waitFor(mutexBase &mutex,
                                          const time::duration d) {
  const auto handle = static_cast<mutex::native_handle *>(mutex.nativeHandle());
...
inline void original::condition::notify() { this->cond_.notify(); }

inline void original::condition::notifyAll() { this->cond_.notifyAll(); }
