 ...
 * @details
 * This header defines the `semaphore` class template and `semaphoreGuard` RAII
 * wrapper for managing concurrent access to shared resources with counting
 * semaphores.
 *
 * The file provides:
 ...
 * - `semaphoreGuard<MAX_CNT>`: RAII wrapper for automatic semaphore
 * acquisition/release
 *
 * Features:
 ...
 *
 * @note Semaphores follow the classic synchronization pattern where acquire()
 * decrements the count and blocks if zero, while release() increments the count
 ...
 * @tparam MAX_CNT Maximum allowed semaphore count (default: 1, binary
 * semaphore)
 * @details
 * A counting semaphore that controls access to a shared resource pool. The
 * semaphore count represents the number of available resources. acquire() waits
 * for an available resource, while release() returns a resource to the pool.
 *
 ...
 */
template <u_integer MAX_CNT = 1> class semaphore : public noMeta {
...
public:
   ...
   * @details
   * Blocks if releasing would exceed MAX_CNT, waiting until space becomes
   * available. Notifies waiting threads after successful release.
   */
   ...
   * @param increase Number of resources to release (default: 1)
   * @return true if resources were released, false if release would exceed
   * MAX_CNT
   */
 ...
 * @details
 * A semaphore with no upper limit on the count. release() operations never
 * block, as there's no constraint on the maximum semaphore value.
 */
template <> class semaphore<0> : public noMeta {
 ...
 * @tparam MAX_CNT Maximum semaphore count (matches semaphore template
 * parameter)
 * @details
 * Acquires the semaphore on construction and automatically releases it on
 * destruction. Ensures exception-safe semaphore usage and prevents resource
 * leaks.
 */
template <u_integer MAX_CNT> class semaphoreGuard : public noMeta {
...
template <original::u_integer MAX_CNT>
original::semaphore<MAX_CNT>::semaphore(const u_integer init_count)
    : count_(init_count) {
  if (init_count > MAX_CNT) {
    throw valueError("Init count is " + printable::formatString(init_count) +
                     ", that is larger than the max count " +
                     printable::formatString(MAX_CNT));
  }
}
...
void original::semaphore<MAX_CNT>::acquire() {
  {
    ...
    this->condition_.wait(this->mutex_, [this] { return this->count_ > 0; });
    this->count_ -= 1;
  }
...
bool original::semaphore<MAX_CNT>::acquireFor(time::duration timeout) {
  ...
  {
    uniqueLock lock{this->mutex_};
    success = this->condition_.waitFor(this->mutex_, timeout,
...
void original::semaphore<MAX_CNT>::release(const u_integer increase) {
  if (increase > MAX_CNT) {
    throw valueError("Increase is larger than max count " +
                     printable::formatString(MAX_CNT));
  }
  {
    uniqueLock lock{this->mutex_};
...
template <original::u_integer MAX_CNT>
bool original::semaphore<MAX_CNT>::releaseFor(const u_integer increase,
...
inline original::semaphore<0>::semaphore(const u_integer init_count)
    : count_(init_count) {}
...
inline bool original::semaphore<0>::acquireFor(const time::duration &timeout) {
  ...
  const bool success = this->condition_.waitFor(
      this->mutex_, timeout, [this] { return this->count_ > 0; });
  if (success) {
    this->count_ -= 1;
  }
  return success;
