...
namespace original {
    ...
    public:
        ...
        socket(sockets::addressFamily af, sockets::type type,
...
inline int original::socket::toNativeAF(const sockets::addressFamily af) {
  switch (af) {
  case sockets::addressFamily::IPV4:
    return AF_INET;
  case sockets::addressFamily::IPV6:
    return AF_INET6;
  default:
    return AF_INET;
  }
...
inline int original::socket::toNativeType(const sockets::type type) {
  switch (type) {
  case sockets::type::STREAM:
    return SOCK_STREAM;
  case sockets::type::DATAGRAM:
    return SOCK_DGRAM;
  }
  return SOCK_STREAM;
}
...
inline int original::socket::toNativeProtocol(const sockets::protocol p) {
  switch (p) {
  case sockets::protocol::TCP:
    return IPPROTO_TCP;
  case sockets::protocol::UDP:
    return IPPROTO_UDP;
  default:
    return 0;
  }
...
inline original::socket::socket(const sockets::addressFamily af,
  ...
  net::initialize();
  this->handle_ =
      ::socket(toNativeAF(af), toNativeType(type), toNativeProtocol(protocol));
...
inline void original::socket::connect(const endpoint &ep) const {
  if (::connect(this->handle_, ep.nativeData(), ep.nativeSize()) != 0) {
    throw std::runtime_error("connect failed");
  }
}

inline void original::socket::setOption(const sockets::option opt,
  ...
  if (setsockopt(this->handle_, level, name,
...
inline void original::socket::bind(const endpoint &ep) const {
  if (::bind(this->handle_, ep.nativeData(), ep.nativeSize()) != 0) {
...
inline std::size_t original::socket::send(constBufferType buffer) const {
  const int sent =
      ::send(this->handle_, reinterpret_cast<const char *>(buffer.data()),
             static_cast<int>(buffer.count()), 0);
...
inline std::size_t original::socket::recv(bufferType buffer) const {
  const int recv =
      ::recv(this->handle_, reinterpret_cast<char *>(buffer.data()),
             static_cast<int>(buffer.count()), 0);
...
inline std::size_t
original::socket::sendAll(const constBufferType buffer) const {
  std::size_t total = 0;
  while (total < buffer.count()) {
    const auto sent = this->send(buffer.subview(total, buffer.count() - total));
    if (sent == 0)
...
inline std::size_t original::socket::recvFull(const bufferType buffer) const {
  ...
  while (total < buffer.count()) {
    const auto received =
        this->recv(buffer.subview(total, buffer.count() - total));
    if (received == 0)
      break;
    total += received;
  }
